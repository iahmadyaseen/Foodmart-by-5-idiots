...
import { Order, OrderStatus, Product, ContactMessage, CategorySlug } from '../types';
...
export const AdminDashboard: React.FC = () => {
  const { user } = useAuth();
  const { products, addProduct, updateProduct, deleteProduct, toggleTopSelling, randomizeTopSelling } = useProducts();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-neutral-900 to-neutral-800 text-white p-8 rounded-3xl shadow-xl">
        <div className="space-y-1">
          ...
          <p className="text-xs text-neutral-400">
            Real-time management for customer orders, catalog stock, and inquiry notifications.
          </p>
        </div>

        <button
          onClick={loadAdminData}
          className="px-4 py-2.5 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-xs font-bold text-neutral-200 border border-neutral-700 transition-colors flex items-center gap-2 self-start sm:self-auto"
        >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#F1E4D8]">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-[#737373]">• Customer: <strong className="text-[#242424]">{order.customerName}</strong> ({order.customerEmail})</span>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <select
                        ...
                        className="bg-white text-xs font-bold text-[#242424] px-3 py-2 rounded-xl border-2 border-neutral-300 focus:border-[#E8483F] focus:outline-none"
                      >
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  ...
                  <div className="pt-2 flex items-center justify-between text-xs gap-2">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                      p.stockQuantity <= 0 ? 'bg-red-100 text-red-800' : p.stockQuantity <= 5 ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
                    }`}>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl border border-[#F1E4D8] p-8 max-w-md w-full shadow-2xl space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowAddProductModal(false)} className="text-[#737373] hover:text-[#242424] cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewProductData({ ...newProductData, stockQuantity: Number(e.target.value) })}
                    className="w-full px-3 py-2 rounded-xl bg-white text-[#242424] font-bold border-2 border-neutral-300 focus:border-[#E8483F] focus:outline-none"
                  />
                </div>
              </div>
